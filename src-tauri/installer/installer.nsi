...
Function .onInit
  ; ---------------------------------------------------------------------
  ; LUNAR: one-click by default.
  ;
  ; Upstream defaults to the wizard and treats /P as opt-in. The only change
  ; here is which way round that is: passive unless /WIZARD is passed. Every
  ; page already carries SkipIfPassive, and the post-install block already
  ; creates the desktop shortcut when passive because the finish page will not
  ; run — so this flips a path Tauri itself wrote and tests, rather than
  ; introducing a new one.
  ;
  ; /P is still honoured, unchanged, because the updater passes it.
  ; ---------------------------------------------------------------------
  StrCpy $PassiveMode 1

  ; Scratch register, not $PassiveMode: GetOptions clears its output variable
  ; when the flag is absent, which would undo the line above on every ordinary
  ; run — a one-character mistake that would silently restore the wizard.
